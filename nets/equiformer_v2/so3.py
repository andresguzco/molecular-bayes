...
class SO3_LinearVariational(torch.nn.Module):
    def __init__(self, in_features, out_features, lmax, bias=True):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        ...
        self.prior_mu = torch.zeros((self.lmax + 1), out_features, in_features)
        self.prior_sigma = torch.ones((self.lmax + 1), out_features, in_features)
        self.prior_log_sigma = torch.zeros((self.lmax + 1), out_features, in_features)

        self.weight_mu = torch.nn.Parameter(torch.randn((self.lmax + 1), out_features, in_features))
        self.weight_log_sigma = torch.nn.Parameter(torch.randn((self.lmax + 1), out_features, in_features))

        self.bias_mu = torch.nn.Parameter(torch.zeros(out_features))
        self.bias_log_sigma = torch.nn.Parameter(torch.zeros(out_features))

        bound = 1 / math.sqrt(self.in_features)
        torch.nn.init.uniform_(self.weight_mu, -bound, bound)
        torch.nn.init.uniform_(self.weight_log_sigma, -bound, bound)
