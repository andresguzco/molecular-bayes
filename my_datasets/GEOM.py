...
class GEOM(InMemoryDataset):
    ...
    def __init__(self, root: str, transform=None, pre_transform=None):
        super(GEOM, self).__init__(root, transform, pre_transform)
        # self.raw_dir = ['scratch/ssd004/scratch/username']
        # self.process()
        # self.processed_paths = ['/scratch/ssd004/scratch/username/rdkit_folder']
        self.data, self.slices = torch.load(self.processed_paths[0])

    @property
    def raw_file_names(self):
        # Specify the tar.gz file name that contains the raw data
        return ['rdkit_folder.tar.gz']
    ...
    def process(self):
        ...
        # rdkit_folder = osp.join(self.raw_dir, 'rdkit_folder')
        rdkit_folder = '/scratch/ssd004/scratch/c/rdkit_folder'
